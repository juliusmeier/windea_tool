...
def plot_weibull(location_dict):
    fig, ax = plt.subplots()
    for loc in location_dict.values():
        loc.df_weibull_detailed.plot(y="h_detailed", ax=ax,
                                     label = loc.name)
...
# Place box under legend or legend below plot?
    ...
    ax.grid()
    #ax.set_title(location.name, fontsize=14)
